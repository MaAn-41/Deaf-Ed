...
const CountingReportScreen = ({ route }) => {
  ...
  const filteredResults = results.filter(
    (item) => selectedNumber === "" || item.number === selectedNumber
  );

  const correctCount = results.filter(
    (item) => item.recognized === item.number
  ).length;
  ...
  // Filter for line chart to include only correct results
  const correctFilteredResults = filteredResults.filter(
    (item) => item.status === "Correct"
  );

  const lineChartData = {
    labels: correctFilteredResults.map((item) =>
      new Date(item.timestamp).toLocaleString()
    ),
    datasets: [
      {
        data: correctFilteredResults.map((item) => item.accuracy),
          ...
          <>
            {!showDetails ? (
              ...
              <>
                ...
                <Text style={styles.chartTitle}>
                  Select a Number to View Accuracy
                </Text>
                <Picker
                ...
                >
                  <Picker.Item label="Select a number" value="" />
                ...
                {selectedNumber && correctFilteredResults.length > 0 ? (
                  <>
                    <Text style={styles.chartTitle}>
                      Accuracy for Number: {selectedNumber}
                    </Text>
                    <View style={styles.chartContainer}>
                      <LineChart
                        data={lineChartData}
                  ...
                  <Text style={styles.noDataText}>
                    No correct data available for "{selectedNumber}".
                  </Text>
                ) : null}
              </>
            ) : (
              ...
              <>
                ...
                <View style={styles.detailPickerContainer}>
                  <Text style={styles.detailPickerLabel}>
                    Filter by Number:
                  </Text>
                  <Picker
                    selectedValue={selectedNumber}
                    ...
                    onValueChange={(itemValue) => setSelectedNumber(itemValue)}
                  >
                    <Picker.Item label="All Numbers" value="" />
                    {uniqueNumbers.map((number, index) => (
                      <Picker.Item
                        key={index}
                        label={number.toString()}
