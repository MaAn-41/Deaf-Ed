...
const SignupScreen = ({ navigation, route }) => {
  ...
  const handleGenerateOtp = async () => {
    try {
      const response = await fetch('http://10.54.9.192:5000/generate-otp', {
        method: 'POST',
  ...
  const handleOtpVerification = async () => {
    ...
    try {
      const response = await fetch('http://10.54.9.192:5000/verify-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSignup = async () => {
    ...
    try {
      const response = await fetch('http://10.54.9.192:5000/signup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email,
